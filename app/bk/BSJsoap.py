...
class BSJsoap:
    ...
    def __init__(self):
        self.client = SoapClient(wsdl=bs["apilink"], trace=False)

    def getBankList(self):
        #获取银行列表soap
        response = self.client.GetBanksList()
    ...
    def checkIfExist(self,clientid):
        ...
        return self.client.CheckClientAccount(clientid)["CheckClientAccountResult"]

    def checkEmail(self,mail):
        """check client's email if exists"""
        return self.client.CheckEmail(mail)["CheckEmailResult"]
    ...
    def insertClient(self,clientAccount,name,password,email,groupsId,phone,address,idCard,bankAccount,currencyId,inputUser,banksId,bankRemark):
        ...
        try:
            result = self.client.InsertClient(clientAccount,name,password,email,int(groupsId),phone,address,idCard,bankAccount,str(currencyId),inputUser,str(banksId),bankRemark)
            #print result
